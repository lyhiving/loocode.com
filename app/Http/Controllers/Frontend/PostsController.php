...
class PostsController extends Controller
{
    ...
    {
        $sql = <<<EOF
SELECT term_taxonomy.taxonomy_id FROM terms JOIN term_taxonomy ON (terms.id = term_taxonomy.term_id) WHERE terms.slug = ? LIMIT 1
EOF;
        ...
        $sql = <<<EOF
SELECT
       t3.id, t3.post_title, t3.post_date, t3.post_excerpt, t3.post_type, t4.name, t4.avatar, t3.post_modified
FROM term_relation as t2
...
EOF;
        $posts = DB::select($sql, [$taxonomy->taxonomy_id]);
        if ($posts) {
    ...
    public function show(Request $request, int $id): View
    {
        ...
        $sql = 'SELECT post_key, post_value FROM postmeta WHERE post_id = ?';
        $metas = DB::select($sql, [$id]);
        $posts->metas = [];
        foreach ($metas as $item) {
            $posts->metas[$item->post_key] = $item->post_value;
        }
        ...
        $sql = <<<EOF
SELECT t1.taxonomy, t2.object_id, t3.slug FROM term_taxonomy as t1 LEFT JOIN term_relation as t2
ON (t1.taxonomy_id=t2.term_taxonomy_id)
...
EOF;
        ...
        $sql = <<<EOF
SELECT
       t2.name, t2.avatar, t1.comment_author, t1.id, t1.comment_date, t1.comment_content
FROM comments AS t1 LEFT JOIN users AS t2
ON (t1.comment_author=t2.id)
WHERE t1.post_id = ? AND (t1.comment_approved = 1 OR comment_author = ?) ORDER BY t1.id
EOF;
        $user = $request->user();
        $userId = $user->id ?? 0;
        $comments = DB::select($sql, [$id, $userId]);
        $author = DB::selectOne('SELECT name, avatar FROM users WHERE id = ?', [$posts->post_author]);
    ...
    public function comment(Request $request, int $id): JsonResponse
    {
        ...
        $comment = [
            ...
            'comment_ip' => $request->getClientIp(),
            'comment_date' => now(),
...
SELECT
   ...
   post_parent, post_order, post_mime_type, comment_count
FROM posts WHERE id = ?
EOF;
